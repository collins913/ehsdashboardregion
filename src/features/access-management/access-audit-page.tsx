...
export function AccessAuditTab() {
  ...
  const query = useLatestAsyncQuery(email, () => loadAuditLog(email));
  ...
  return <div className="space-y-4"><label className="block w-72 space-y-1 text-sm">邮箱搜索<Input value={email} onChange={(event) => setEmail(event.target.value)} placeholder="操作人或授权对象" /></label>{query.status === "ERROR" ? <p role="alert" className="text-sm text-destructive">日志加载失败。</p> : null}
    <DataTable rows={events} columns={columns} emptyMessage="暂无操作日志。" status={query.status === "SUCCESS" ? "READY" : query.status === "ERROR" ? "ERROR" : "LOADING"} semanticKey={email} />
