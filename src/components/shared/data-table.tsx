...
import { dataTableClassName, dataTableColumnSizeClassNames, dataTableFrameClassName, dataTableRowClassName, type DataTableColumnSizeRole } from "@/components/shared/data-table-layout";
...
export function DataTable<T extends { id: string }>({ rows, columns, emptyMessage, status = "READY", semanticKey = "", active = true, interactionState, onInteractionStateChange }: { rows: readonly T[]; columns: readonly DataTableColumn<T>[]; emptyMessage: string; status?: "READY" | "LOADING" | "ERROR"; semanticKey?: string; active?: boolean; interactionState?: DataTableInteractionState; onInteractionStateChange?: Dispatch<SetStateAction<DataTableInteractionState>> }) {
  ...
  return <>
    <div ref={tableFrameRef} className={dataTableFrameClassName} aria-busy={status === "LOADING"} inert={status === "LOADING" ? true : undefined}><Table className={dataTableClassName}>
      ...
      <TableBody ref={tableBodyRef}>{!adaptivePagination ? <TableRow ref={rowMeasurementRef} aria-hidden="true" data-adaptive-table-measurement-row className="pointer-events-none invisible hover:bg-transparent"><TableCell colSpan={columns.length}><div className="h-8" /></TableCell></TableRow> : status === "ERROR" || (status === "LOADING" && renderedRows.length === 0) ? <DataTablePlaceholderRows columns={columns.map((item) => ({ id: item.id, className: dataTableColumnSizeClassNames[item.sizeRole] }))} rowCount={pagination.pageSize} hidden={status === "ERROR"} /> : renderedRows.map((row, index) => <TableRow key={row.id} ref={index === 0 ? rowMeasurementRef : undefined} aria-hidden={status === "LOADING" || undefined} className={cn(status !== "LOADING" && dataTableRowClassName, status === "LOADING" && "hover:bg-transparent")}>{row.getVisibleCells().map((cell) => <TableCell key={cell.id} className={cn(dataTableColumnSizeClassNames[columns.find((item) => item.id === cell.column.id)?.sizeRole ?? "standard"], "break-words")}><DataTableLoadingCellContent loading={status === "LOADING"} pendingMode={pendingMode}><table.FlexRender cell={cell} /></DataTableLoadingCellContent></TableCell>)}</TableRow>)}{status === "LOADING" && renderedRows.length > 0 ? <DataTablePlaceholderRows columns={columns.map((item) => ({ id: item.id, className: dataTableColumnSizeClassNames[item.sizeRole] }))} rowCount={Math.max(0, pagination.pageSize - renderedRows.length)} /> : null}{status === "READY" && displayedRows.length === 0 && adaptivePagination ? <TableRow ref={rowMeasurementRef}><TableCell colSpan={columns.length} className="py-8 text-center text-muted-foreground">{emptyMessage}</TableCell></TableRow> : null}</TableBody>
    </Table></div>
    <div ref={paginationRef} aria-hidden={!adaptivePagination} className={cn("mt-3 flex flex-wrap items-center justify-between gap-3", !adaptivePagination && "invisible")}>
      <span className="text-sm text-muted-foreground">共 <DataTablePendingValue pending={isResolvedMetadataPending}>{displayedRows.length}</DataTablePendingValue> 条</span>
      <div className="flex items-center gap-2">
        <span className="text-sm text-muted-foreground">第 <DataTablePendingValue pending={isResolvedMetadataPending}>{pagination.pageIndex + 1}</DataTablePendingValue> / <DataTablePendingValue pending={isResolvedMetadataPending}>{Math.max(1, table.getPageCount())}</DataTablePendingValue> 页</span>
        <Button size="sm" variant="outline" disabled={status !== "READY" || !table.getCanPreviousPage()} onClick={() => table.previousPage()}>上一页</Button>
        <Button size="sm" variant="outline" disabled={status !== "READY" || !table.getCanNextPage()} onClick={() => table.nextPage()}>下一页</Button>
      </div>
    </div>
  </>;
}
